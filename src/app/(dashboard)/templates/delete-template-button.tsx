...
export function DeleteTemplateButton({ templateId }: { templateId: string }) {
  ...
  async function handleDelete() {
    try {
      ...
      if (error) {
        // Penanganan error Foreign Key jika template sudah pernah digunakan di invoice
        if (error.code === "23503") {
          setErrorMessage(
            "Template tidak dapat dihapus karena sudah pernah digunakan untuk mencatat transaksi."
          );
        } else {
  ...
  return (
    <>
      ...
      <button
        ...
        className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-red-600 hover:bg-red-50 border border-red-200 rounded-lg transition-colors cursor-pointer focus:outline-none focus:ring-2 focus:ring-red-500/20 disabled:opacity-50"
      >
        <Trash2 size={14} />
        <span>Hapus</span>
      </button>
            ...
            <div className="flex flex-col items-center text-center space-y-3">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-500 leading-relaxed">
                  Tindakan ini tidak dapat dibatalkan. Seluruh konfigurasi kolom dan rumus pada template ini akan hilang.
                </p>
              </div>
            </div>
